...
function displayLine(
  line: string[],
  correctnessArr: number[],
  enableColor?: boolean
): ReactNode[] {
  const wordArr: ReactNode[] = [];
  line.forEach((word, idx) => {
    const correctness = correctnessArr[idx];
    if (correctness === 0 || !enableColor)
      wordArr.push(<span style={{ color: "white" }}>{word} </span>);
    if (correctness === 1 && enableColor)
      wordArr.push(<span style={{ color: "green" }}>{word} </span>);
    if (correctness === 2 && enableColor)
      wordArr.push(<span style={{ color: "red" }}>{word} </span>);
  });
  return wordArr;
}
...
export default function TypingTest() {
  ...
  const [correctnessArr, setCorrectnessArr] = useState(
  ...
  if (hasGameEnded && calculateWpm() >= CHEAT_LIMIT)
    return (
      <ChristmasWrapper>
        <div
          style={{
          ...
          <ChristmasContainer>
            <div className={classes.textDiv}>
              <p className={classes.subtitle}>
                ...{calculateWpm()} ord i minuttet?
              </p>
              <p className={classes.snif1}>*snif*</p>
              <p className={classes.snif2}>*snif*</p>
              <p>
                Det lukter <span className={classes.zoomTxt}>juks</span> her O:
              </p>
        ...
        >
          <ChristmasContainer>
            <div className={classes.textDiv}>
              ...
              <p>
                Du skrev hele{" "}
                <span className={classes.zoomTxt}>{calculateWpm()}</span> ord i
                minuttet O:
              </p>
              <p>Koden er... {CODE}❤️</p>
            </div>
        ...
        >
          <ChristmasContainer>
            <div className={classes.textDiv}>
              ...
              <p>
                For å få koden til låsen må du nemlig greie å skrive mer enn{" "}
                <b>50 ord i minuttet</b>.
              </p>
            ...
            <div className={classes.flexContainer}>
              ...
              {hasGameEnded && calculateWpm() < 51 && (
                <>
                  <button className={classes.btn} onClick={restartGame}>
                    <b>Prøv igjen &lt;3</b>
                  </button>
                  <p>{calculateWpm()} ord/min</p>
                </>
